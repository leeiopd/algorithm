...
A, B, C 3개의 사탕 포장이 있을 때 이 포장들을 한번에 하나로 합칠 수는 없고, 한번에 2개씩만 합칠 수 있다. 예를 들면 A와 B를 먼저 합친 후 C를 다시 합치거나 A와 C를 먼저 합치고 B를 합치기,

혹은 B와 C를 먼저 합치고 A를 합칠 수 있다.
 
사탕 포장을 풀었다가 다시 합치는 순서는 매우 중요한데, 그 이유는 그 순서에 따라 전체 비용이 달라지기 때문이다.
 
사탕 포장 A와 B에 각각 a개와 b개의 사탕이 들어있다고 할 때 이 둘을 합치는 비용은 a + b라고 한다.

그러므로 A와 B를 먼저 합치고 C를 합치는 경우 그 전체 비용은 a + b + a + b + c이며, B와 C를 먼저 합치고 A를 합치는 비용은 b + c + b + c + a 이므로 그 둘은 서로 같지 않을 수 있다.
...
첫 번째 줄에는 포장 개수 N(1<=N<=3000)이 주어진다.
두 번째 줄에는 포장에 들어있는 사탕의 개수 ni(1<=ni<=100)가 주어진다.
'''
N = int(input())

ni_list = list(map(int, input().split()))
result = 0
...
for k in range(N-1):
    ...
    for i in range(k, k+2):
        for j in range(i+1, N):
            if ni_list[i] > ni_list[j]:
                ni_list[i], ni_list[j] = ni_list[j], ni_list[i]
    ni_list[k+1] += ni_list[k]
    result += ni_list[k+1]
